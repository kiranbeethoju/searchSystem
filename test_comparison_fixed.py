...
def test_bi_encoder(model_name, query, procedures):
    ...
    try:
        ...
        procedure_embeddings = model.encode(procedures, convert_to_tensor=True, device='cpu')
...
def test_hybrid_reranker(retriever_model, reranker_model, query, procedures, top_k=5):
    ...
    try:
        # Stage 1: Retrieval - Force CPU
        retriever = SentenceTransformer(retriever_model)
        procedure_embeddings = retriever.encode(procedures, convert_to_tensor=True, device='cpu')
        query_embedding = retriever.encode(query, convert_to_tensor=True, device='cpu')
        
        cos_scores = util.cos_sim(query_embedding, procedure_embeddings)[0]
        
        # Convert to numpy for argpartition
        cos_scores_np = cos_scores.cpu().numpy() if hasattr(cos_scores, 'cpu') else cos_scores.numpy()
        top_results_indices = np.argpartition(cos_scores_np, -top_k)[-top_k:]
...
def improved_matching_check(query_desc, procedures, top_n=3):
    """Improved matching logic to check if expected result is in top N"""
    query_lower = query_desc.lower()
    
    # Define key terms for each query type
    key_terms = {
        'ckd stage 4': ['stage 4', 'stage iv'],
        't2dm with nephropathy': ['type 2', 'diabetic nephropathy'],
        'hfref': ['reduced ejection', 'hfref'],
        'hap': ['hospital-acquired', 'hospital acquired'],
        'htn stage 2': ['stage 2', 'stage ii']
    }
    ...
    # Check if any of the top N procedures contain the relevant terms
    for i in range(min(top_n, len(procedures))):
        proc_lower = procedures[i].lower()
        if any(term in proc_lower for term in relevant_terms):
            return True
...
def run_comprehensive_test():
    ...
    # Model configurations - using working model identifiers
    models_config = {
        "ClinicalBERT": "emilyalsentzer/Bio_ClinicalBERT",
        "PubMedBERT": "NeuML/pubmedbert-base-embeddings",
        "BioBERT": "dmis-lab/biobert-v1.1"  # Alternative working model
    }
    ...
    for query, query_desc in test_queries:
        ...
        for model_name, model_path in models_config.items():
            ...
            result = test_bi_encoder(model_path, query, medical_procedures)
            
            if result[0] is None:
                print(f"Error with {model_name}: {result[1]}")
                continue
            
            results, exec_time = result
            print(f"Execution Time: {exec_time:.2f} seconds")
            print("Top 5 Results:")
            for i, (procedure, score) in enumerate(results[:5]):
                print(f"{i+1}. {score:.4f} - {procedure}")
            
            # Check if the expected match is in top 3 using improved logic
            top_3_procedures = [proc for proc, _ in results[:3]]
            expected_found = improved_matching_check(query_desc, top_3_procedures)
        ...
        result = test_hybrid_reranker(retriever_model, reranker_model, query, medical_procedures)
        
        if result[0] is None:
            print(f"Error with Hybrid Reranking: {result[1]}")
        else:
            results, exec_time = result
            print(f"Execution Time: {exec_time:.2f} seconds")
            print("Top 5 Results:")
    ...
    print("\nBest performing results for each query:")
    queries_grouped = {}
    for result in results_summary:
        query = result['query']
        if query not in queries_grouped:
            queries_grouped[query] = []
        queries_grouped[query].append(result)
    
    for query, results in queries_grouped.items():
        print(f"\n{query}:")
        # Sort by whether expected was found, then by score
        results.sort(key=lambda x: (x['expected_in_top3'], x['top_score']), reverse=True)
        best = results[0]
        print(f"  Best: {best['model']} ({best['approach']}) - Score: {best['top_score']:.4f}, Found Expected: {best['expected_in_top3']}")
        print(f"  Top Result: {best['top_result']}")
